...
export class ProblemsPanel {
	...
	constructor() {
		...
		Store.on(STORE_KEYS.composingInProcess, () => this.updatePanelTitle());
		Store.on(STORE_KEYS.scanningInProcess, () => this.updatePanelTitle());
		Store.on(STORE_KEYS.scanningFolder, () => this.updatePanelTitle());
		Store.on(STORE_KEYS.uploadInProgress, () => this.updatePanelTitle());
		Store.on(STORE_KEYS.uploadCompleted, () => this.updatePanelTitle());
		Store.on(STORE_KEYS.analysisInProgress, () => this.updatePanelTitle());
		Store.on(STORE_KEYS.analysisCompleted, () => this.updatePanelTitle());
		Store.on(STORE_KEYS.analysisResults, () => this.updatePanelTitle());
	...
	createPanelTitle() {
		...
		const scanning = 'Analyse process is not started yet. Scannig folders...';
		...
		if (isComposing || isUpload || isAnalysis) {
			return processing;
		}

		if (!analyseIsCompleted || isScanning) {
			return scanning;
		}
