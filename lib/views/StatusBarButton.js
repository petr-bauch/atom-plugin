...
export class StatusBarButton {
  ...
  init(statusBar) {
    ...
    Store.on(STORE_KEYS.scanningInProcess, (eventData) => {
      this.onChangeScanningInProgress(eventData);
      this.render();
    });
    Store.on(STORE_KEYS.scanningFolder, () => this.render());
    Store.on(STORE_KEYS.uploadInProgress, (eventData) => {
      this.onChangeUploadInProgress(eventData);
      this.render();
    });
    Store.on(STORE_KEYS.uploadCompleted, () => this.render());
    Store.on(STORE_KEYS.analysisInProgress, (eventData) => {
      this.onChangeAnalysisInProgress(eventData);
      this.render();
    });
    Store.on(STORE_KEYS.analysisCompleted, () => this.render());
    Store.on(STORE_KEYS.composingInProcess, (eventData) => {
      this.onChangeComposingInProgress(eventData);
      this.render();
    });
    Store.on(STORE_KEYS.analysisResults, () => {
      this.onChangeAnalysisResults();
      this.render();
    });
    Store.on(STORE_KEYS.analysisWasStarted, () => {
      this.onChangeAnalysisResults();
      this.render();
    });
  }
