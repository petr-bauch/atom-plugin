...
export class StatusBarButton {
  ...
  renderCount(totalCount, criticalCount, warningCount, infoCount) {
    if (totalCount === 0) {
      const analysisWasStarted = Store.get(STORE_KEYS.analysisWasStarted);
      ...
      return this.renderIndicator(SEVERITY.ok);
    }

    const counts = [];
    if (criticalCount > 0) {
      const indicator = this.renderIndicator(SEVERITY.critical, criticalCount);
      counts.push(indicator);
    }
    if (warningCount > 0) {
      const indicator = this.renderIndicator(SEVERITY.warning, warningCount);
      counts.push(indicator);
    }
    if (infoCount > 0) {
      const indicator = this.renderIndicator(SEVERITY.info, infoCount);
      counts.push(indicator);
    }
