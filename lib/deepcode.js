...
export default {
  ...
  async initialize() {
    ...
    Logger.critical(`DB connected: ${dbConnected}`);
    ...
    Logger.critical('Activating plugin');
    Logger.critical('Shared State: ', sharedState);
    Logger.critical('Project State: ', projectState);
  ...
  initSubscriptions() {
    this.subscriptions = new CompositeDisposable(
      ...
      atom.config.observe(`${PLUGIN.name}.debugLog`, () => {
        Logger.init();
      }),

      // observe config changes: 'debugLogFile'
      atom.config.observe(`${PLUGIN.name}.debugLogFile`, () => {
        Logger.init();
      }),
  ...
  serialize() {
    ...
    if (now - this.lastStateLog > 1000 * 60) {
      ...
      Logger.critical('Serialize. Shared State:', sharedState);
      Logger.critical('Serialize. Project State:', projectState);
