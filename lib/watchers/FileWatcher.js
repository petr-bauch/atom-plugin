...
class FileWatcher {
  ...
  async activate() {
    ...
    this.fileWatcher = atom.project.onDidChangeFiles(async events => {
      for (const event of events) {
        ...
        console.log(`FileWatcher: file "${path}" is ${action}`);

        if (action === FILE_ACTIONS.renamed) {
          const oldFileInfo = await this.getFileInfo(oldPath);
          this.changedFiles[oldPath] = oldFileInfo;
        }

        const fileInfo = await this.getFileInfo(path);
        this.changedFiles[path] = fileInfo;
      };
    });
  ...
  removeFolderToScan(folder) {
    this.foldersToScan = this.foldersToScan.filter(folderPath => folderPath !== folder);
  }
  ...
  async scanFolder(folder) {
    ...
    for (const filePath of folderList) {
      ...
      if (isDir) {
        ...
        this.changedFiles[fullPath] = fileInfo;
      }
    };
  ...
  async getFileInfo(filePath) {
    const fileInfo = this.changedFiles[filePath];
