...
export class CommonUtils {
  ...
  static clonePluginConfig() {
    ...
    const accountType = sessionToken ? 'free' : '';

    Store.setMany({
      [STORE_KEYS.serviceURL]: serviceURL,
      [STORE_KEYS.sessionToken]: sessionToken,
      [STORE_KEYS.accountType]: accountType,
    });
  }

  // Problems Tree

  static makeFileRowID(fileName) {
    return `row-file-${fileName}`;
  }

  static makeDescriptionRowID(fileName, startRow) {
    return `row-description-${fileName}-${startRow}`;
  }

  static makeMarkerRowID(fileName, startRow, startCol, suggestionID) {
    return `row-marker-${fileName}-${startRow}-${startCol}-${suggestionID}`;
  }
