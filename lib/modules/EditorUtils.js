...
class EditorUtils {
  ...
  getMarkerRow(fileName, markerID) {
    const { table } = Store.get(STORE_KEYS.analysisResults);
    ...
    for (const desc of descriptions) {
      const markerRow = find(desc.markers, { markerID });
      if (markerRow) {
        return markerRow;
      }
    }
  ...
  onClickIgnoreProblem(eventData) {
    ...
    const fileName = editor.getPath();
    const markerRow = this.getMarkerRow(fileName, markerID);
    if (!markerRow) {
      this.hideMarkerOverlay(fileName);
      return;
    }

    const { startRow, startCol, suggestionID } = markerRow;
