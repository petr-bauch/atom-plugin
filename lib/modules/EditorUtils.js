...
class EditorUtils {
  ...
  showMarker(event) {
    ...
    const { marker } = appropriateFlattenMarker;

    const model = this.getProblemModel(fileName, marker);
    if (!model) {
      return;
    }

    model.top = clientY + 16;
    model.left = clientX - 22;
  ...
  getProblemModel(fileName, marker) {
    ...
    const { descriptions } = fileRow;
    for (const descriptionRow of descriptions) {
      if (descriptionRow.startRow === startRow && descriptionRow.startCol === startCol) {
        const newMarker = find(descriptionRow.markers, { startRow, startCol });
        if (!newMarker) {
          continue;
        }
        ...
        return {
          ...
          message: descriptionRow.message,
          severity: descriptionRow.severity,
        };
      }
    }
  ...
  async openFile(path = '', row = 1, col = 1, markerID = null) {
    ...
    this.hideMarkerOverlay(path);

    // if there is no markerID, it means user has clicked on Description row,
    // so we just make mouse event and show marker overlay
    if (!markerID) {
    ...
    // otherwise we have to check: is this marker equal to base description row
    const marker = {
      startRow: row,
      startCol: col,
    };
    const model = this.getProblemModel(path, marker);
    if (!model) {
      return;
    }
    if (model.markerID === markerID) {
      const mouseEvent = this.createMouseEventForBufferPosition(editor, position);
      this.showMarker(mouseEvent);
    }
  }
