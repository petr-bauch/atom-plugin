...
const maxPayload = 3.5 * 1024 * 1024;
...
class BundleModule {
  ...
  async startBundleLoop(ignoreConfigOptionAnalyseOnChange = false) {
    if (!ignoreConfigOptionAnalyseOnChange) {
      const analyseOnChange = CommonUtils.analyseOnChange();
      if (!analyseOnChange) {
        Logger.log('Analyse on change not allowed');
        return Promise.resolve();
      }
    }
  ...
  async createBundle() {
    ...
    if (statusCode !== 200) {
      Logger.error(BUNDLE_ERRORS.create[statusCode]);
      Logger.log(error);
      return Promise.resolve(null);
    }
  ...
  async checkBundle(bundleID) {
    ...
    if (statusCode === 401 || statusCode === 403) {
      Logger.error(BUNDLE_ERRORS.check[statusCode]);
      Logger.log(error);
      return Promise.resolve(null);
    }
  ...
  async extendBundle(bundleID) {
    ...
    if (statusCode !== 200) {
      Logger.error(BUNDLE_ERRORS.extend[statusCode]);
      Logger.log(error);
      return Promise.resolve(null);
    }
