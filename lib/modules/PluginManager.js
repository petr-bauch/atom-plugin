...
class PluginManager {
  ...
  openProblemsPanel() {
    setTimeout(async () => {
      await atom.workspace.open(PLUGIN.problemsPanelURI);
  ...
  scanProject() {
    ...
    if (confirmedFolders.length === 0) {
      this.checkConfirmedFolders(true);
      return;
    }
  ...
  checkConfirmedFolders(showWarning = false) {
    // If project has no paths (it may be when we open a single file in Atom)
    // we can't use plugin properly
    const projectPaths = FileUtils.getProjectPaths();
    if (isEmpty(projectPaths)) {
      Logger.log('No project paths found');
      if (showWarning) {
        atom.notifications.addWarning('You have no opened project so far', {
          dismissable: true,
        });
      }
      return;
    ...
    const onClickConfirm = (folder) => {
      ...
      this.checkConfirmedFolders(showWarning);
    };
