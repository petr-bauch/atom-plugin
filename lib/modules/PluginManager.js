...
class PluginManager {
  ...
  checkFilesToUpload() {
    const count = FileWatcher.getChangedFilesCount();
    if (!count) {
      return;
    }
    ...
    const onClickUpload = () => {
      ...
      this.confirmUploadDialog.dismiss();
    }

    this.confirmUploadDialog = atom.notifications.addInfo(MESSAGES.confirmUploadPrompt(count), {
      dismissable: true,
      buttons: [
        {
          text: MESSAGES.confirmUploadOK,
          onDidClick: onClickUpload,
        },
      ],
    });
  }
