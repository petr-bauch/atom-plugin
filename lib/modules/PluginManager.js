...
class PluginManager {
  ...
  checkFilters() {
    setTimeout(async () => {
      ...
      const { extensions, configFiles } = await HttpModule.getFilters(sessionToken);
  ...
  didUpdateConfigAnalyseOnChange() {
    setTimeout(async () => {
      if (BundleModule.loopTimeout) {
        clearTimeout(BundleModule.loopTimeout);
      }

      await BundleModule.startBundleLoop();
    }, 0);
  }
